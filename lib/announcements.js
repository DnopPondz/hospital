  ...
  if (missingSlugDocuments.length > 0) {
    await Promise.all(
      missingSlugDocuments.map(async (doc) => {
        ...
        const fallbackBase = baseFromTitle || `announcement-${doc._id.toString()}`;
  ...
  if (statusUpdates.length > 0) {
    await Promise.all(
      statusUpdates.map(({ id, status }) =>
        collection.updateOne({ _id: id }, { $set: { status } })
      )
    );
  }
...
export async function updateAnnouncement(slug, updates = {}) {
  ...
  const { _id: existingId, ...existingFields } = existing;
  const payload = { ...existingFields };
  ...
  if (
    ...
    Object.prototype.hasOwnProperty.call(updates, 'displayFrom') ||
    Object.prototype.hasOwnProperty.call(updates, 'displayUntil') ||
    existingFields.status !== normalizedPayload.status ||
    !Object.prototype.hasOwnProperty.call(existingFields, 'status')
  ) {
    updateDoc.status = normalizedPayload.status;
  }

  if (Object.keys(updateDoc).length === 0) {
    const normalizedExisting = normalizeAnnouncement(existingFields);

    if (existingFields.status !== normalizedExisting.status) {
      await collection.updateOne({ _id: existingId }, { $set: { status: normalizedExisting.status } });
    }
  ...
  if (imageToRemove && imageToRemove !== normalized.imageUrl) {
    try {
      ...
      console.warn('Failed to delete replaced announcement image', error);
    }
  }
