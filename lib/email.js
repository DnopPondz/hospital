...
function createConnection({ host, port, secure, timeout }) {
  return new Promise((resolve, reject) => {
    const socket = secure
      ? tls.connect({ host, port, timeout })
      : net.createConnection({ host, port, timeout });
...
async function sendMail({
  ...
  const socket = await createConnection({ host, port, secure, timeout: 15000 });
  const state = { buffer: '' };

  try {
    await sendCommand(socket, state, null, [220]);
    await sendCommand(socket, state, `EHLO ${escapeHeaderValue(process.env.SMTP_CLIENT_NAME || 'localhost')}`, [250]);
